...
set-account1-configuration:
	aws configure --profile user-account1-profile
...
set-account2-configuration:
	aws configure --profile user-account2-profile

create-test-stack:
	ROLE=$$(aws cloudformation describe-stacks --stack-name test-useraccount --query 'Stacks[0].Outputs[?OutputKey==`CloudformationRole`].OutputValue' --output text) \
	...
	aws cloudformation wait stack-create-complete --stack-name test-stack --profile user-account1-profile
	@echo "Stack Created."
update-test-stack:
	ROLE=$$(aws cloudformation describe-stacks --stack-name test-useraccount --query 'Stacks[0].Outputs[?OutputKey==`CloudformationRole`].OutputValue' --output text) \
